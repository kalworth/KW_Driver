...
static inline void float_plus_to_1K_data(float val, uint8_t *data)
{
    int16_t val_plus=val*1000.0f;
    data[0] = *(((uint8_t*)(&val_plus)) + 0);
    data[1] = *(((uint8_t*)(&val_plus)) + 1);
}
static inline float data_1k_to_float(uint8_t *data)
{
	float tmp_float;
	int16_t val=(int16_t)((uint16_t)data[0] |((uint16_t)data[1]<<8));
	tmp_float=(float)val/1000.0f;
	return tmp_float;
}
