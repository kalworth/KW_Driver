...
{
	DEBUG("Commands:\n\r");
	DEBUG(" r   - System Reset\n\r");
	DEBUG(" p   - Get Pos\n\r");
	DEBUG(" x   - Check Config\n\r");
	DEBUG(" v   - Get Vbus\n\r");
	DEBUG(" m   - Motor Mode\n\r");
	DEBUG(" c   - Calibration\n\r");
	DEBUG(" z   - Clear Err\n\r");
...
static void print_setup(void)
{
	...
	DEBUG("  inertia = %f\n\r", UsrConfig.inertia);
	DEBUG("  Foc.adc_phase_a_offset = %d\n\r", Foc.adc_phase_a_offset);
	DEBUG("  Foc.adc_phase_b_offset = %d\n\r", Foc.adc_phase_b_offset);
...
//	USART_SendArray((uint8_t*)Fsm.cmd_buff,Fsm.bytecount);
	Fsm.bytecount = 0;
	memset(Fsm.cmd_buff, 0, sizeof(Fsm.cmd_buff));
}
